...
void checkTaskTimers() {
  ...
  for (int i = 0; i < currentLength; i++) {
    ...
    if (currentMillis - startedStreakTimes[i] >= interval
       	&& streakArray[i] != 0) {  // Check if timer has ran out     
    ...
    } else if (currentMillis - startedStreakTimes[i] >= interval - notificationTime
               && streakArray[i] != 0) {
      TetrisTheme();
    }
  }
